...
function handler_roundrobin(image)
    ...
    if not rootdevice then
        swupdate.error("Cannot determine current root device from /proc/cmdline.")
        return 1
    end
    ...
    if chained_handler == "rdiff_image" then
        if image.properties ~= nil and image.properties["rdiffbase"] ~= nil then
            ...
            err, image.properties["rdiffbase"], _ = get_device_path(rdiffbase_devices[rrtarget.index])
            if err ~= 0 then
                return 1
            end
    ...
    if config.bootloader.name == BOOTLOADER.EBG then
        -- Update EFI Boot Guard environment: kernelparams
